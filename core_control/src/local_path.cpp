...
void LocalPath::avoid_obstacle_path(PC_XYZI &path){
    ...
    {
        for(size_t i=0; i< frenet_path.size() / 2; i++){
...
////            fprintf(stderr,"asdf : %lf", hypot(path[0].x-p.x,path[0].y-p.y));
                frenet_time = ros::Time::now();
                if ((frenet_state == 1 || hypot(curr_pose.position.x-p.x, curr_pose.position.y-p.y)<15.0)){
                    if (frenet_state == 1 && pre_frenet_state == 0)
                        frenet_start = ros::Time::now();
                    avoid_obstacle_path(path);
                    pre_frenet_state = frenet_state;
                    frenet_state = 1; // frenet local start
                    return path;
                }
